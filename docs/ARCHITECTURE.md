...
- The finalizer ships a row only when it covers every canonical word, has
  ...
  measured voice, and fits inside its exact MP3. It fills holes from a same-clock
  reference, otherwise uses a complete quran-align fallback. If neither is
  physically valid, the row is withheld and the reader honestly highlights the
  whole ayah.
