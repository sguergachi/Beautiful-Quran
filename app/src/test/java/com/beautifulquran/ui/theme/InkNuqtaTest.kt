...
class InkNuqtaTest {
    ...
    fun `every layer starts dry and ends fully spread`() {
        NuqtaLayer.entries.forEach { layer ->
    ...
    fun `wet edge leads the body, and the body leads the pool`() {
        clock.filter { it > 0f && it < 1f }.forEach { t ->
            ...
            val body = nuqtaReach(t, NuqtaLayer.Body)
            val pool = nuqtaReach(t, NuqtaLayer.Pool)
    ...
    fun `wetting is quick and the settle is soft`() {
        ...
        assertTrue(nuqtaReach(0.33f, NuqtaLayer.WetEdge) > 0.8f)
        // …while the pool is still creeping into the corners in the last fifth.
        val lateReach = nuqtaReach(1f, NuqtaLayer.Pool) - nuqtaReach(0.8f, NuqtaLayer.Pool)
